...
c = {k:v for (x, y) in centroids for v = lambda x: distance(location, x)]

    c = [(x, y) for x, y in centroids] 
    a = {k:v for k in c for v distance(location, k)}
    return min(a)

    c = [(x, y) for x, y in centroids] 
    a = {k:distance(location, k) for k in c}
    return min(a)



return min(letters, key=lambda k: letters[k])  

    for i in centroids:
        if i not in a:
            a[i] = distance(location, i)
